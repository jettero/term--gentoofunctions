...
BTW, L<Term::GentooFunctions> will use C<RC_INDENTATION> and
C<RC_DEFAULT_INDENT> from /sbin/functions.sh...  So you can C<eindent> in a
bash_script.sh and your perl_script.pl will use the indent level!  However, to
get it to work you must ...

    export RC_INDENTATION RC_DEFAULT_INDENT 
    
... before you fork to perl.  Also, L<Term::GentooFunctions> won't be able to
modify the indent level in a way that will propagate back up to bash
(obviously).
